...
const corsOptions = {
origin: ['https://ruletfront.vercel.app', 'https://wishyroulette.onrender.com', 'http://localhost:10000',
'http://localhost:3000'],
...
if (totalPotAdjustment < 0n) { let totalPot=Object.values(contributors).reduce((a, b)=> a + BigInt(b), 0n);
  totalPot += totalPotAdjustment;
  if (totalPot <= 0n) { console.log('Outgoing transfer detected, checking if payout...'); const lastPayout=await
    getState('lastPayout', null); if (!lastPayout || lastPayout.amount !==(-totalPotAdjustment).toString()) {
    console.log('Not a payout, preserving contributors'); // Don't reset contributors } else { contributors={};
    console.log('Payout confirmed, reset contributors'); } } else { const scale=totalPot / (totalPot -
    totalPotAdjustment); for (const sender in contributors) {
    contributors[sender]=(BigInt(Math.floor(Number(BigInt(contributors[sender])) * Number(scale)))).toString(); if
    (BigInt(contributors[sender])===0n) delete contributors[sender]; } } } if (data.length> 0) {
    ...
    if (rand >= cum && rand < cum + amt) { winner=addr; break; } cum +=amt; } if (winner) { console.log(`Winner
      selected: ${winner}`); const success=await payoutTo(winner); if (success) { console.log(`Payout of
      ${pot.toString()} WISHY to ${winner} completed`); contributors={}; lastWinner=winner; await
      setState('contributors', contributors); await setState('lastWinner', lastWinner); await setState('lastPayout', {
      winner, amount: pot.toString(), timestamp: Date.now() }); return { success: true, winner }; } else { return {
      success: false, error: 'Payout failed' }; } } else { console.log('No winner selected (edge case)'); return {
      success: false, error: 'No winner' }; } } else { console.log('Pot is empty, no lottery'); return { success: true,
      message: 'Empty pot' }; } } else { console.log(`No new block or initial height. Current: ${height}`); }
      currentHeight=height; await setState('currentHeight', currentHeight); return { success: true, message: 'No action'
      }; } catch (e) { console.error('Check block error:', e); return { success: false, error: e.message }; } } async
      function payoutTo(winnerAddress) { console.log(`Attempting payout to ${winnerAddress}`); if (!runeId ||
      !runeId.includes(':')) { console.error(`Payout aborted: runeId invalid (${runeId})`); return false; } try { const
      utxoRes=await fetch(`${baseUrl}/address/${address}/utxos`, { headers }); if (!utxoRes.ok) { const errorText=await
      utxoRes.text(); console.error(`UTXO fetch failed: HTTP ${utxoRes.status}, ${errorText}`); return false; } const
      utxoResponse=await utxoRes.json(); const utxos=utxoResponse.data || []; if (utxos.length===0) { console.log('No
      UTXOs'); return false; } let totalSats=0n; let hasRune=false; let runeAmount=0n; utxos.forEach(u=> {
      ...
      if (change < 0n) { console.log('Insufficient sats for fee'); return false; } const psbt=new bitcoin.Psbt({ network
        }); const p2wpkh=bitcoin.payments.p2wpkh({ pubkey: Buffer.from(child.publicKey), network }); for (const u of
        utxos) { if (!u.outpoint) { console.error(`Invalid UTXO: ${JSON.stringify(u)}`); continue; } const [h,
        idxStr]=u.outpoint.split(':'); const script=u.script_pubkey ? Buffer.from(u.script_pubkey, 'hex' ) :
        p2wpkh.output; psbt.addInput({ hash: h, index: Number(idxStr), witnessUtxo: { value: Number(BigInt(u.value)),
        script } }); } psbt.addOutput({ address: winnerAddress, value: Number(dust) }); let outputIndex=0; if (change>=
        dust) {
        ...
        for (let i = 0; i < utxos.length; i++) { psbt.signInput(i, signer); } psbt.finalizeAllInputs(); const
          tx=psbt.extractTransaction(); const txHex=tx.toHex(); console.log(`Built TX: ${txHex}`); const broadRes=await
          fetch(`${mempoolBase}/tx`, { method: 'POST' , body: txHex }); if (broadRes.ok) { const txid=await
          broadRes.text(); console.log(`Broadcast successful: TXID ${txid}`); return true; } else { const
          errorText=await broadRes.text(); console.error(`Broadcast failed: ${errorText}`); return false; } } catch (e)
          { console.error(`Payout error: ${e.message}`); return false; } } async function getBalance() {
          console.log(`Fetching rune balance for address: ${address}`); try { let res=await
          fetch(`${baseUrl}/address/${address}/runes`, { headers }); if (!res.ok) { console.warn(`Ordiscan failed: HTTP
          ${res.status}, trying Hiro...`); res=await
          fetch(`https://api.hiro.so/ordinals/v1/addresses/${address}/runes-balances`); } if (!res.ok) { const
          errorText=await res.text(); console.error(`Balance fetch failed: HTTP ${res.status}, ${errorText}`); return {
          success: false, error: errorText }; } const response=await res.json(); const data=response.data ||
          response.results || []; let balance=0n; for (const rune of data) { if (rune.rune_name===runeName ||
          rune.name===runeName) { balance=BigInt(rune.amount || rune.balance); break; } } console.log(`Current rune
          balance: ${balance.toString()} WISHYWASHYMACHINE`); return { success: true, balance: balance.toString() }; }
          catch (e) { console.error(`Balance fetch error: ${e.message}`); return { success: false, error: e.message }; }
          } function encodeVarint(n) { const bytes=[]; if (n===0n) return Buffer.from([0]); while (n> 0n) {
